RegisterNetEvent('crimeticket:notifyAdmin', function(senderSrc, senderName)
    ...
    local timer = GetGameTimer() + 10000 -- 10 секунд
    local firstShow = true
    local drawY = 0.88 -- отступ от нижнего края
    ...
    Citizen.CreateThread(function()
        while display and GetGameTimer() < timer do
            ...
            if IsControlJustReleased(0, keyE) then
                -- Наблюдать через ps-adminmenu
                TriggerServerEvent('crimeticket:forceSpectate', senderSrc)
                display = false
